...
export default function PropertySearchContent({ dict }: { dict: Dictionary }) {
  ...
  // Search State (Initialized with URL param)
  ...
  const performSearch = useCallback(async () => {
    ...
    try {
      ...
      // Handle Geocoding if there is a query text
      if (searchQuery.trim()) {
        // Simple check: if it looks like a coordinate or specific place, we might want to geocode
        // For now, we reuse your logic: try to geocode the query string
        const coords = await getLatLng(searchQuery);
        if (coords) {
          targetLatLng = `${coords.lat}, ${coords.lng}`;
          targetRadius = 5000;
          finalQueryText = ""; // Clear text query if we found a location match to rely on geo-search
          useGeoSearch = true;
        } else {
          // If geocoding fails, treat it as a normal text search
          useGeoSearch = false;
        }
      } else {
        // No query, show all (geo search around default KL center)
        useGeoSearch = true;
        targetRadius = "all"; 
      }
  ...
  }, [performSearch, searchQuery, filters]); // Re-run when query or filters change
